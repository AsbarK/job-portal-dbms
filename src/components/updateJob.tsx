...
export default function UpdateJobForm({jobId}:any) {
  ...
  function onSubmit(data: z.infer<typeof formSchema>) {
    ...
    axios.post(`${process.env.NEXT_PUBLIC_URL_LINK}/api/updateJob`,{
      ...data,startDate:format(data.startDate,"yyyy-MM-dd"),endDate:format(data.endDate,"yyyy-MM-dd"),jobId
    }).then(()=>(
      toast.success('Successfully Created Job'),
      router.push('/allEmpJobs')
    )).catch(()=>(
  ...
  return (
    <Form {...form}>
      <div className="flex flex-col justify-center items-center h-9/10 mr-8 space-x-4">
        <form
          onSubmit={form.handleSubmit(onSubmit)}
          className=" w-9/10 border border-border rounded-md p-4"
        >
